...
size_t cgi::parse(const string& qs) {
    ...
    while (std::getline(iss, pair, '&')) {
        ...
        if (pos != std::string::npos) {
            ...
            if (key == "air_temp" || key == "dewpoint") {
                kvp.emplace(key, decoded.substr(pos+1));
            } else {
                json j;
                j["message"] = "CGI Error: Unknown QUERY_STRING parameter.";
                j["expected"] = {"air_temp", "dewpoint"};
                j["actual"] = key;
                j["status"] = "error";
